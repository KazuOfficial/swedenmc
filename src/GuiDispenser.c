#include "Renderer.h"

#include "GuiDispenser.h"
#include "Minecraft.h"
#include "ContainerDispenser.h"
#include "FontRenderer.h"
#include <stdlib.h>

static GuiScreenVtable s_vtable;
static int s_vtable_init = 0;

static void s_vtable_ensure_init(void) {
    if (s_vtable_init)
        return;
    GuiContainer_initVtable();
    s_vtable = GuiContainer_vtable;
    s_vtable.drawGuiContainerForegroundLayer =
        (void (*)(GuiScreen *))GuiDispenser_drawGuiContainerForegroundLayer;
    s_vtable.drawGuiContainerBackgroundLayer =
        (void (*)(GuiScreen *, float))GuiDispenser_drawGuiContainerBackgroundLayer;
    s_vtable_init = 1;
}

void GuiDispenser_construct(GuiDispenser *self, InventoryPlayer *var1, TileEntityDispenser *var2) {
    ContainerDispenser *container = calloc(1, sizeof(ContainerDispenser));
    ContainerDispenser_construct(container, (struct IInventory *)var1, var2);
    GuiContainer_construct(&self->base, (Container *)container);
    s_vtable_ensure_init();
    self->base.base.vtable = &s_vtable;
}

void GuiDispenser_drawGuiContainerForegroundLayer(GuiDispenser *self) {
    FontRenderer_drawString(self->base.base.fontRenderer, "Dispenser", 60, 6, 4210752);
    FontRenderer_drawString(self->base.base.fontRenderer, "Inventory", 8, self->base.ySize - 96 + 2,
                            4210752);
}

void GuiDispenser_drawGuiContainerBackgroundLayer(GuiDispenser *self, float var1) {
    (void)var1;
    unsigned int var2 = RenderEngine_getTexture(self->base.base.mc->renderEngine, "/gui/trap.png");
    R_color4f(1.0F, 1.0F, 1.0F, 1.0F);
    RenderEngine_bindTexture(self->base.base.mc->renderEngine, var2);
    int var3 = (self->base.base.width - self->base.xSize) / 2;
    int var4 = (self->base.base.height - self->base.ySize) / 2;
    Gui_drawTexturedModalRect((Gui *)self, var3, var4, 0, 0, self->base.xSize, self->base.ySize);
}
